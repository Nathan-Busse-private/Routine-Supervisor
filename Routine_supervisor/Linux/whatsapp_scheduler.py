...
database_directory_path = 'events.txt'
modified_database_directory_path = 'modified_events.txt'

# Main function
def main():
    current_script_path = os.path.dirname(os.path.abspath(__file__))
    base_path = os.path.join(current_script_path, '...')  # Moves up 2 level to the Routine_supervisor folder
...
text_files = ['events.txt', 'modified_events.txt', 'maintenance_mode_actions.txt', 'power_loss_actions.txt', 'deleted_messages_log.txt', 'messages.txt']
for file in text_files:
    ...
    database_directory_path = os.path.join(database_directory_path, 'Database')
...
def read_events_from_file(database_directory_path):
    events = []
    if os.path.exists(database_directory_path):
        with open(database_directory_path, 'r') as file:
            events = file.read().splitlines()
    return events

def write_events_to_file(database_directory_path, events):
    with open(database_directory_path, 'w') as file:
        file.write('\n'.join(events))
...
def is_media_file(database_directory_path):
    media_extensions = ['.gif', '.jpg', '.jpeg', '.png']
    _, file_extension = os.path.splitext(database_directory_path)
    return file_extension.lower() in media_extensions

def add_event_to_file(event):
    # Read existing events from the file
    events = read_events_from_file(database_directory_path)

    # Append the new event
    events.append(event)

    # Write the updated events to the file
    write_events_to_file(database_directory_path, events)
